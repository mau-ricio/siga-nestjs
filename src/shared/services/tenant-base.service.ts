  ...
  implements OnModuleInit
{
  ...
  constructor(
    ...
    if (!this.request) {
      ...
      throw new Error(
        'Request object not available in TenantBaseService constructor.',
      );
    }
    ...
    this.logger.debug(
      `Tenant ID from middleware property: ${tenantIdFromMiddleware}`,
    );

    if (!tenantIdFromMiddleware) {
      this.logger.error(
        'Tenant ID was not set by middleware on the request object.',
      );
      throw new Error(
        'Tenant ID is missing from the request (not set by middleware).',
      );
    }
  ...
  async onModuleInit(): Promise<void> {
    ...
    try {
      ...
      const tenantConnection =
        await this.connectionProviderService.getConnection(this.tenantId);
      ...
      this.logger.log(
        `Repository successfully initialized for tenant: ${this.tenantId}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to initialize repository for tenant ${this.tenantId}: ${error.message}`,
      );
      throw new Error(
        `Could not initialize tenant repository: ${error.message}`,
      );
    }
  }
  ...
  protected async ensureRepositoryInitialized(): Promise<void> {
    if (!this.repository) {
      this.logger.log(
        `Repository not initialized for tenant ${this.tenantId}, initializing now...`,
      );
